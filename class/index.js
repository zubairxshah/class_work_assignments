...
function inArray(rest, ...anyThing) {
...
let recursion = (numr) => {
    if (numr === 20)
        return console.log(numr + " found.");
    if (numr < 20)
        return console.log(numr + " comes before 20.");
    recursion(numr + 1);
};
recursion(3);
function matchingLetter(str, letter, index = 0) {
    if (index === str.length)
        return null;
    if (str[index] === letter)
        return index;
...
function indexingLetters(message, index) {
    if (index === 0) {
        console.log('letter is ', message[0]);
        return;
    }
...
function IncrementNums(nums) {
    if (nums === 50)
        return; // Base Condition: if nums reaches 50 stop the code
...
let stringArray = create2DArray(3, 3, 'empty');
