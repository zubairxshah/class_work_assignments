...
function countingVowels (getString : any) {
  let vowelcounter = 0;
...
function vowelsCount (vowel:any) {
...
function isPrime(num:number) { // function to check true or false
    if (num <= 1) return false; // check for numbers below 1
    if (num % 2 == 0 && num > 2) return false; // check for even numbers using remainder method
    ...
    for(let i = 3; i <= sroot; i += 2) { // starting a loop from 3 with an increment of 2 of squareroot times
        if(num % i === 0) return false; // if loop finds a 0 remainder result will be false
    }
    return true; // true if num is a prime number
  }
  
    const number = await inquirer.prompt ([ // getting user input in number format
...
function firstUniqueChar(str : string): string | void {
  ...
  let numInput = 10
  function squareNum(numInput : number) {
    numInput *= numInput;
    return numInput;
  }
...
const postParameter = (post :'Hello World', end : ' See you') => console.log (post + end + ' As parameters');
